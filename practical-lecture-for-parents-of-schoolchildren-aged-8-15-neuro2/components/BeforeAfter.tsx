...
const rows: Array<{ before: string; after: string }> = [
  {
    before: nbsp('Платили репетиторам до 150 000 ₽ в год'),
    after: nbsp(
      'ИИ-помощник за 0 ₽. Не даёт готовые ответы, а объясняет тему столько раз, сколько нужно',
    ),
  },
  {
    before: nbsp('Ребёнок проводил время в телефоне без пользы'),
    after: nbsp('Использует ИИ как инструмент — учится, создаёт, решает задачи'),
  },
  {
    before: nbsp('Опасались, что ИИ научит списывать'),
    after: nbsp('Видите разницу между копированием и осмысленным использованием'),
  },
  {
    before: nbsp('Не знали, как объяснить ребёнку непонятную тему'),
    after: nbsp('Включаете ИИ-репетитора прямо по нужной теме в учебнике'),
  },
];
          ...
          {rows.map((r, i) => (
            ...
            >
              <div className="rounded-xl border border-ink-200 bg-white/60 p-6 md:p-8">
                ...
                <p className="mt-3 text-body-lg text-ink-700">{r.before}</p>
              </div>
              ...
              <div className="card !shadow-cardHover">
                ...
                <p className="mt-3 text-body-lg font-medium text-ink-900">{r.after}</p>
              </div>
            </article>
          ))}
        </div>
